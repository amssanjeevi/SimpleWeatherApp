...
extension SessionsManager {
    ...
    func getUrlRequest(url: URL) -> NSMutableURLRequest {
        ...
        request.timeoutInterval = Constants.TimeLimits.RequestTimeOut
    ...
    private func verifyAndExtractData(data: Any?, response: Any?, error: Error?, completion: Success?) {
        guard error == nil else {
            Notifier.shared.showAlert(alertTitle: "Error", message: error?.localizedDescription ?? "", firstButtonTitle: "Okay")
            return
        }
        guard let response = response as? HTTPURLResponse else {
            Notifier.shared.showAlert(alertTitle: "Empty Response", message: "Invalid or Empty Response from the Server", firstButtonTitle: "Okay")
            return
        }
        guard let data = data as? Data else {
            Notifier.shared.showAlert(alertTitle: "Invalid Data", message: "Invalid or Empty Data from the Server", firstButtonTitle: "Okay")
            return
        }
        let serialisedData = data.serialise
        guard (200...299) ~= response.statusCode else {
            ...
            Notifier.shared.showAlert(alertTitle: "Bad Response Code", message: errorName + errorMessage, firstButtonTitle: "Okay")
            return
        }
